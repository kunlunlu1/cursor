...
2. 202103-1 序列查询（数学规律推导，O(n) 优化）
3. 202209-1 如此编码（混合进制解码，理解即可）
4. 202109-1 数组推导（贪心思维，读懂推导过程）
5. **202309-1 坐标变换(I)**（三角函数模拟，注意度→弧度转换）🆕

**第二阶段（目标 300 分）**：攻克 T2 题（掌握一两种 T2 算法即可）
6. 202012-2 期末预测之最佳阈值（排序+枚举，逻辑清晰）
7. 202109-2 非零段划分（差分数组技巧）
8. **202303-2 垦田计划**（二分答案入门题，思路清晰）🆕
9. 202203-2 训练计划（拓扑排序入门）
